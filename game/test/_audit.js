...
const MUST = [
  ...
  /* ── 형님이 눈으로 찾아낸 것들 (2026-08-11) ─────────────────────────── */
  ["index.html", /#file,#dir,#zip\{ position:fixed;/, 1, "파일칸을 전부 화면 밖으로"],
  ["index.html", /font-size:24px; font-weight:700;/, 1, "A·B 글자를 버튼 크기에 맞춤"],
  ["index.html", /\.caplbl\{ position:absolute; font-size:11px;/, 1, "SELECT·START 이름표를 읽을 수 있게"],
  ["index.html", /border-left:2px solid var\(--or\);/, 1, "게임화면 테두리는 좌우선만"],
  ["index.html", /ADD GAME\$\{CAN_DIR \? " FOLDER" : ""\}/, 2, "폴더를 여는 기기에서는 FOLDER 라고 적음"],
  ["game.js", /this\.imageData\.data\.set\(this\.frameBuf\);\n    if \(!this\.colorReal\) tintOrange/, 1, "칠하기는 매번 원본에서 (지직거림 방지)"],
  ["game.js", /removed: true,/, 1, "지워도 세이브는 남김"],
  ["game.js", /if \(v\.removed\) \{ c\.continue\(\); return; \}/, 1, "지운 게임은 목록 훑기에서 제외"],
  ["ui.js", /parts\.push\(dup \+ " ALREADY IN" \+ \(names \? " \(" \+ names \+ "\)" : ""\)\);/, 1, "중복된 게임 이름을 알려줌"],
  ["ui.js", /notice = \(screen === "play"\) \? "" : \(colorReal/, 1, "게임 화면에는 색 안내를 안 남김"],
  ["ui.js", /SAVE KEPT/, 1, "지워도 세이브가 남는다고 알려줌"],
